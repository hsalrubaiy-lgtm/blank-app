...
# 1. الخانات ليفوق (الأرباح والمصاريف والصافي أول شي بالشاشة)
...
with tab2:
    ...
    if not df.empty:
        ...
        with col_g1:
            fig1 = px.pie(df, values='المبلغ (د.ع)', names='التصنيف الرئيسي', title="نسبة الواردات للمصاريف")
...
with tab4:
    ...
    # ميزة التصدير (خلاصة ليفوق والتفاصيل جوة)
    if not df.empty:
